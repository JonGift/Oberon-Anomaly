...
class Frigate:
    def __init__(self, ship_type, thruster, display_surf, team = 0, hp_max = 100, delete_hp = -3, shields_max = 100, shield_regen_rate = 0.1):
        self.name = ('Temp Frigate Name')
        self.type_name = 'Frigate'
        self.type_reference = 'Frigate'
    ...
    def update_ship(self):
        if self.hp > 0:
            if self.shields < self.shields_max:
                self.shields += self.shield_regen_rate
            if self.rotation < 0:
            ...
            self.display_surf.blit(self.thruster_final, (self.rect))
            self.hitmask = get_alpha_hitmask(self.ship_final, self.rect, 0)
            self.speed = 0
            if self.shields > 0:
                if self.previous_hp > self.hp:
                    self.shields -= (self.previous_hp - self.hp)
                    self.hp = self.previous_hp
                    if self.shields < 0:
                        self.hp += self.shields
            self.previous_hp = self.hp
        else:
            self.move_calc()
            self.display_surf.blit(self.ship_destroyed, (self.rect))